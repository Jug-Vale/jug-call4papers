...
public class InscricaoResourceImpl implements InscricaoResource {
	...
	public Response inscrever(long eventoId, Participante participante) {
		ResponseBuilder rb;
		Evento evento = lanca404SeNulo(eventoService.buscarPorId(eventoId), eventoId);
		if (!evento.isInscricoesAbertas()) {
			return Response.status(Status.FORBIDDEN).entity("Inscrições já passaram").build();
		}
		Participante partipanteExistente = participanteService.buscaPorEmail(participante.getEmail());
		...
		Inscricao inscricao = eventoService.buscaInscricao(evento, participante);
		if (inscricao != null) {
			rb = Response.status(Status.CONFLICT);
		} else {
			inscricao = eventoService.inscreverParticipante(evento, participante);
			rb = Response.ok();
	...
	public Response anulaInscricao(long id) {
		...
		return Response.ok("Inscrição de '" + inscricao.getParticipante().getEmail() + "' no evento '"
				+ inscricao.getEvento().getNome() + "' anulada.").build();
	}
