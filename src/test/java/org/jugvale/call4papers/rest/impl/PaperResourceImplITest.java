...
public class PaperResourceImplITest {
	...
	private static final String PAPER_CONTEXT = SERVICES_CONTEXT.concat("/paper");
	...
	public void setUp() throws Exception {
			paper = Paper.newPapper().aceito().comAutor(Autor.newAutor().build())
									 .comDescricao("DESCRICAO").comTitulo("")
									 .miniCurso().noEvento(Evento.newEvento().build())
									 .submetidoEm(new Date()).build();
	...
	public void deveRetornarPaperEmBuscaPorId() throws Exception {
		...
		get(PAPER_CONTEXT + "/7").then().assertThat().
												body(notNullValue()).
	...
	public void deveRetornarUnauthorizedParaCriarNovoPaper() {
		given().
			contentType(JSON).body(jsonPaper).
		when().
			post(PAPER_CONTEXT).
		then().
